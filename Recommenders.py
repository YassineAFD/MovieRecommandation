...
class svd_recommender_py():
    ...
    def get_user_items(self, user):
        user_data = self.train_data[self.train_data['userId'] == user]
    ...
    def recommend(self, user): 
        ...
        train_data_matrix = self.train_data.pivot_table(index=['userId'],columns=['movieId'],values='rating',fill_value=0)
        u, s, vt = svds(train_data_matrix, self.k)
        s_diag_matrix=np.diag(s)
        X_pred = np.dot(np.dot(u, s_diag_matrix), vt)
        recom = []
        count = 1
        for movie in (-X_pred[user, :]).argsort():
            if movie not in user_movies and count <= 10:
                recom.append(movie)
                count = count + 1
        return recom
    ...
    def generate_top_recommendations(self, user, cooccurence_matrix, all_movies, user_movies):
        ...
        for i in range(0,len(sort_index)):
            if ~np.isnan(sort_index[i][0]) and all_movies[sort_index[i][1]] not in user_movies and rank <= 10:
                df.loc[len(df)]=[user,int(all_movies[sort_index[i][1]]),sort_index[i][0],rank]
                rank = rank+1
